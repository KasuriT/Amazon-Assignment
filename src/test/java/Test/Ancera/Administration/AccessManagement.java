...
public class AccessManagement{
	...
	public void NavigateAm() throws InterruptedException, IOException {
		NavigateToScreen.navigate(url_access, "Access Management", accessTitle);
	}
			...
			for (int j=1; j<rows;j++) {
				...
				//	int rows1 = driver.findElements(By.cssSelector("td:nth-child(5) label")).size();
					for (int k=1;k<rows;k++) {
						if (driver.findElement(By.cssSelector("tr:nth-child("+k+") td:nth-child(5) label")).getText().equals(config.ie_username())) {
	...
	public void OrganizationSitesManagementAccess() throws InterruptedException, IOException {
		try{
			...
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("notification-loading")));
			Thread.sleep(1000);
			softAssert.assertEquals(driver.findElements(OrganizationManagementPage.orgAddSite1).size(), 0, "Add Org site button is not hidden");
			driver.findElement(OrganizationManagementPage.orgSite1Click).click();
			Thread.sleep(2000);
