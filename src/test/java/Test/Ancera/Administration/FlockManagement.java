...
public class FlockManagement extends BaseTest {
    ...
    public String getFarmNameMethod() throws SQLException {
        ...
        while (getFarmNameResults.next()) {
            farmName = getFarmNameResults.getString("siteName");
    ...
    public void CreateFlock() throws InterruptedException, IOException, SQLException {
        try {
            ...
            if (Constants.config.url().contains("qa") || Constants.config.url().contains("dev")) {
                ...
                System.out.println(Queries.getFarmHouseCount(getFarmNameMethod()));

                ResultSet getHouseNameResults = DB_Config_DW.getStmt().executeQuery(Queries.getFarmHouseCount(getFarmNameMethod()));
    ...
    public void DuplicateFlockWithSameSiteAndPlacementDate() throws InterruptedException, IOException, SQLException {
        try {
            ...
            if (Constants.config.url().contains("qa") || Constants.config.url().contains("dev")) {
                ...
                //		System.out.println("Farm Name: "+farmName);
                click(flockFarmDropdownExpand);
                type(flockFarmDropdownSearch, getFarmNameMethod());
            ...
            test.pass("Duplicate Flock created successfully");
            getScreenshot();
            saveResult(ITestResult.SUCCESS, null);
        } catch (AssertionError er) {
            test.fail("Duplicate Flock failed to create successfully");
            saveResult(ITestResult.FAILURE, new Exception(er));
        } catch (Exception ex) {
            test.fail("Duplicate Flock failed to create successfully");
            saveResult(ITestResult.FAILURE, ex);
...
//			type(flockInlineProgramName, FlockManagementModel.flockProgramName);
    ...
    @Test(priority = 43)
    public void ExportCSVAudit() throws InterruptedException, IOException {
        getDriver().get(url_flockManagement);
        waitElementInvisible(loading_cursor);
        Thread.sleep(3000);
        CSVAuditExport("Flock Management", flockPlacementAuditFileName, flockPlacementTable, false);
    }
