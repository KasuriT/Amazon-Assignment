...
public class DataTemplateManagement{
	...
	public void extent() throws InterruptedException, IOException {
		spark = new ExtentSparkReporter("target/Reports/MetaData_Template_Management"+date+".html");
	...
	public void Navigate() throws InterruptedException, IOException {
		NavigateToScreen(url_dataTemplate, "Data Template Management", DataTemplateManagementReportPath, dataTemplateTitle);
	}
	...
	public void ResetFieldCheck() throws InterruptedException, IOException {
		try {
			...
			Thread.sleep(1000);
			test.addScreenCaptureFromPath(getScreenshot("Data Template Management", DataTemplateManagementReportPath));
			...
			test.addScreenCaptureFromPath(getScreenshot("Data Template Management", DataTemplateManagementReportPath));
			saveResultNew(ITestResult.SUCCESS, DataTemplateManagementReportPath, null);
		}catch(AssertionError er) {
			test.fail("Template fields failed to reset");
			results.createNode("Template fields failed to reset");
			saveResultNew(ITestResult.FAILURE, DataTemplateManagementReportPath, new Exception(er));
		}
		catch(Exception ex) {
			test.fail("Template fields failed to reset");
			results.createNode("Template fields failed to reset");
			saveResultNew(ITestResult.FAILURE, DataTemplateManagementReportPath, ex);
		}
	}	
	...
	public void CreateTemplate() throws InterruptedException, IOException {
		try {		
			...
			softAssert.assertEquals(driver.findElement(By.cssSelector("tr:nth-child(1) td:nth-child(3) label")).getText(), dateMMDDYYYY1);
			softAssert.assertAll();
	...
	public void IdentificationColumnTest() throws InterruptedException, IOException {
		try {
			...
			preconditions.createNode("4. Click on MetaData and select Data Template Management; Data Template Management screen opens");
			preconditions.createNode("5. Create a new template");
			preconditions.createNode("6. Add identification column");
			steps.createNode("1. Go to data upload screen");
			steps.createNode("2. Verify uploading template with no value in identification column is allowed");
			...
			SoftAssert softAssert = new SoftAssert();
			driver.findElement(By.id("file-input")).sendKeys(projectPath+"/Excel/IndentificationFieldCheck.xlsx");
			waitElementInvisible(loading_cursor);
			...
			results.createNode("Identification field check verified successfully");
			saveResultNew(ITestResult.SUCCESS, DataTemplateManagementReportPath, null);
		}catch(AssertionError er){
			test.fail("Identification field check failed to verify");
			results.createNode("Identification field check failed to verify");
			saveResultNew(ITestResult.FAILURE, DataTemplateManagementReportPath, new Exception(er));
		}
		catch(Exception ex) {
			test.fail("Identification field check failed to verify");
	...
	public void EndOfFlockColumnsVerification() throws InterruptedException, IOException {
		try{
			test = extent.createTest("AN-DTM-18: Verify end of flock column verfication");
			preconditions = test.createNode(Scenario.class, PreConditions);
			steps = test.createNode(Scenario.class, Steps);
			results = test.createNode(Scenario.class, Results);
