		...
		try{
			for(int i=0; i<Test_Variables.lstProfileNavigate.size();i++) {
				try {
					...
					Test_Elements.wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("notification-loading")));
					Thread.sleep(500);
					Assert.assertEquals(Helper.driver.findElement(By.cssSelector("#screen-header p")).getText(), Test_Variables.lstProfileNavigate.get(i).pageTitle);
					...
					Test_Variables.test.addScreenCaptureFromPath(Helper.getScreenshot("Profile Setting", Constants.ProfileSettingReportPath));
					Helper.saveResultNew(ITestResult.SUCCESS, Constants.ProfileSettingReportPath, null);
				}
				catch(AssertionError er) {
					Test_Variables.test.fail("User did not navigated to Profile Setting page");
					Test_Variables.results.createNode("User did not navigated to Profile Setting page");
					Helper.saveResultNew(ITestResult.FAILURE, Constants.ProfileSettingReportPath, new Exception(er));
				}			
			}
		}
		catch(Exception ex){
			Test_Variables.test.fail("User did not navigated to Profile Setting page");
			Test_Variables.results.createNode("User did not navigated to Profile Setting page");
			Helper.saveResultNew(ITestResult.FAILURE, Constants.ProfileSettingReportPath, ex);
		}	
	}
