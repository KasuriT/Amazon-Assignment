...
public class FlockManagement extends DB_Config {
	...
	while (getComplexNameResults.next()) {
		String complexName = getComplexNameResults.getString("site_name");
		...
		waitElementInvisible(loading_cursor);
		Thread.sleep(1000);
		click(By.xpath("//*[text() = '"+complexName+"']"));
	}
	...
	public void CreateFlock() throws InterruptedException, IOException, SQLException {
		try {
			...
			while (getFarmNameResults.next()) {
				String farmName = getFarmNameResults.getString("site_name");
			...
			click(flockEstHarvestDateCalendar);
			DateUtil.getDay("15");
			...
			List<WebElement> a = driver.findElements(flockHousePlacedList);
			a.get(0).click();
	...
	public void EditFlock() throws InterruptedException, IOException {
		try {
			...
			softAssert.assertEquals(getRowsPost+1, getRowsPre);
			softAssert.assertAll();

			/*
				Set<String> uniqueProductName = new HashSet<String>();
				driver.findElements(By.cssSelector("td:nth-child(8)"))
				.stream()
			...
			//	uniqueProductName.forEach(name -> System.out.println(name));
				...
				softAssert.assertEquals(uniqueProductName.size(), driver.findElement(By.id(ResultsCount)).getText(), "Did not displayed unique flock id");								
				softAssert.assertAll();
			 */
	...
	@Test (enabled= true, priority =12) 
	public void InlineEditFlock() throws InterruptedException, IOException {
		try {			
			...
			type(flockInlineProgramName, "");			
			type(flockAdministrationMethod, "");
			click(flockProgramSaveButton);
			type(flockBirdBreed, "Male");
			...
			type(flockInlineMortality1Input, "1.5");
