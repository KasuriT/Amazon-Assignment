...
public class ReportsManagement {
	...
	public void extent() throws InterruptedException, IOException {
		spark = new ExtentSparkReporter("target/Reports/Administration_Reports_Management"+date+".html");
	...
	public void NavigateSalmonella() throws InterruptedException, IOException {
		NavigateToScreen(url_reportsManagement, "Reports Management", ReportManagementReportPath, reportsManagementTitle);
	}

	
	@Test (priority = 2, enabled = true) 
	public void LockFilter() throws InterruptedException, IOException {
		Lock(reportsManagementTable, "Reports Management", ReportManagementReportPath, 0);
	}
	...
	public void OpenPopup() throws InterruptedException, IOException {
		try{
			...
			test.addScreenCaptureFromPath(getScreenshot("Report Management", ReportManagementReportPath));
			Thread.sleep(1000);
			...
			test.addScreenCaptureFromPath(getScreenshot("Report Management", ReportManagementReportPath));
			saveResultNew(ITestResult.SUCCESS, ReportManagementReportPath, null);
		}		
			...
			saveResultNew(ITestResult.FAILURE, ReportManagementReportPath, new Exception(er));
		}
		catch(Exception ex) {
			test.fail("Create Role popup window did not open successfully");
			results.createNode("Create Role popup window did not open successfully");
			saveResultNew(ITestResult.FAILURE, ReportManagementReportPath, ex);
		}
	}
	...
	public void AuditTrail() throws InterruptedException, IOException {
		try{
			...
			waitElementInvisible(loading_cursor);
			type(By.id(userEmail+""+SearchInput), login_email);
			waitElementInvisible(loading_cursor);
			Thread.sleep(3000);
			...
			waitElementInvisible(loading_cursor);
			Thread.sleep(1000);
			String ReportRole = driver.findElement(By.cssSelector("tr:nth-child(1) #col-"+userReportingCol+" label")).getText();
	...
	public void VerifyInActiveRoleUsers() throws InterruptedException, IOException {
		try{
			...
			driver.get(url_user);
				waitElementInvisible(loading_cursor);
			wait.until(ExpectedConditions.visibilityOfElementLocated(usercreateButton));
			Thread.sleep(3000);
	...
	public void MandatoryReportGroupFieldCheck() throws InterruptedException, IOException {
		...
		for (RMModel objModel : lstRGMandatoryCheck) {
